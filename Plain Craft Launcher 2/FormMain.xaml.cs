    ...
    public void PageChange(PageStackData stack, PageSubType subType = PageSubType.Default)
    {
        if (string.IsNullOrEmpty(PageNameGet(stack)))
        {
            ...
            switch (stack.page)
            {
                ...
                {
                    if (ModMain.frmServerLeft is null)
                        ModMain.frmServerLeft = new PageServerLeft();
                    foreach (var item in ModMain.frmServerLeft.PanItem.Children)
    ...
    private void PageChangeActual(PageStackData stack, PageSubType subType)
    {
        ...
        try
        {
            ...
            switch (stack.page)
            {
                ...
                case PageType.Server: // 服务器
                    {
                        ModMain.frmServerLeft ??= new PageServerLeft();
                        subType = ModMain.frmServerLeft.pageID;
                        PageChangeAnim(ModMain.frmServerLeft, (FrameworkElement)ModMain.frmServerLeft.PageGet(subType));
                        break;
                    }
