...
public partial class PageDownloadInstall
{
    ...
    // 判断 Fabric API 是否兼容（传入 vanillaName）
    public bool IsFabricApiCompatible(ModComp.CompFile fabricApi, string vanillaName)
    {
        if (fabricApi?.RawGameVersions == null || string.IsNullOrEmpty(vanillaName))
            return false;

        try
        {
            var normalizedVanilla = NormalizeVersion(vanillaName);
            return fabricApi.RawGameVersions.Any(v =>
                string.Equals(v, normalizedVanilla, StringComparison.OrdinalIgnoreCase));
        }
        catch (Exception ex) when (ex is NullReferenceException or ArgumentException)
        {
            ModBase.Log(ex, $"判断 Fabric API 版本适配性出错: {fabricApi.DisplayName}, {vanillaName}");
            return false;
        }
    }

    // 获取过滤后的 Fabric API 版本列表（传入 vanillaName）
    private List<ModComp.CompFile> GetFilteredFabricApiVersions(string vanillaName)
    {
        var versions = new List<ModComp.CompFile>();

        // 确保加载器已初始化
        if (ModDownload.dlFabricApiLoader?.output == null)
            return versions;

        foreach (var version in ModDownload.dlFabricApiLoader.output)
        {
            if (IsFabricApiCompatible(version, vanillaName))
            {
                // 修改显示名（仅当未加前缀时）
