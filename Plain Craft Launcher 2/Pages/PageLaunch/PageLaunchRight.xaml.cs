...
public partial class PageLaunchRight : IRefreshable
{
    ...
    private void BtnHintClose_Click(object sender, EventArgs e)
    {
        ...
        // 待匹配的关键词列表（已小写）
        List<string> targetWords = new List<string> { "wrh", "wrh666", "xinghai", "星海" };

        // 判断是否包含任意一项（子串包含）
        if (targetWords.Any(word => input.Contains(word)))
        {
            ModAnimation.AniDispose(PanHint, true);
            States.Hint.CEMessage = false;
        }
        else if (input.Contains("xinhai"))
        {
            HintService.Hint(Lang.Text("Launch.Right.CommunityHint.XinghaiWrongInput"));
            ModAnimation.AniDispose(PanHint, true);
            States.Hint.CEMessage = false;
        }
        else
